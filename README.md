...
The `202` response contains `document_id` and `job_id`. Complete that job with the manual simulator, which reads the same `INTERNAL_SERVICE_TOKEN` configured for the API:
...
The full contract, polling design, tradeoffs, and deliberate omissions are in [`docs/`](docs/). Current limitations include upload idempotency, concurrency stress coverage, CI, and the React UI. No real clinical summarization, automatic queue consumer, OCR, or malware scanning is planned for this assessment.
